...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    input_not_tensor = False
    if not isinstance(imgs, torch.Tensor):
        print ('INPUT NOT TENSOR', imgs.shape)
        input_not_tensor = True
    if isinstance(imgs, torch.Tensor) and len(imgs.shape)>3:
        pass
    ...
    for scale in scales:
        im_data = imresample(imgs.to(dtype = torch.float32), (int(h * scale + 1), int(w * scale + 1))).to(dtype = imgs.dtype)
...
def crop_resize(img, box, image_size):
    if isinstance(img, np.ndarray):
        ...
        if len(img.shape)<=3:
            img = img[:, box[1]:box[3], box[0]:box[2]]
            out = imresample(img.unsqueeze(0), (image_size, image_size)).squeeze(0)
        else:
            out_list=[]
            for k in range(img.shape[0]):   
                coord_1 = int(max(box[k,1], 0))
                coord_3 = int(max(box[k,3], 0))
                coord_0 = int(max(box[k,0], 0))
                coord_2 = int(max(box[k,2], 0))
                
                img_temp = img[k:k+1,:,coord_1:coord_3, coord_0:coord_2]
                
                out_temp = imresample(img_temp, (image_size, image_size))
...
def get_size(img):
    if isinstance(img, (np.ndarray, torch.Tensor)):
        return (img.shape[2], img.shape[2])
    else:
        return img.size


def extract_face(img, boxes, image_size=160, margin=0, save_path=None):
    ...
    if not isinstance (boxes, torch.Tensor):
        ...
        if boxes.shape[0]!=img.shape[0]:
            return torch.zeros((img.shape[0], 3, image_size, image_size))
    if len(boxes.shape)>1:
        face = crop_resize(img, boxes, image_size)
