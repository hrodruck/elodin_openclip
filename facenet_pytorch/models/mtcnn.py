...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_prob=False):
        ...
        if isinstance(img, torch.Tensor):
            img_for_box = img.clone().detach()
        else:
            img_for_box = img
        # Detect faces
        batch_boxes, batch_probs, batch_points = self.detect(img_for_box, landmarks=True)
        if not isinstance(batch_boxes, torch.Tensor): #detection failed
            return torch.zeros((img.shape[0], img.shape[1], self.image_size, self.image_size))
        # Select faces
        if not self.keep_all:
            batch_boxes, batch_probs, batch_points = self.select_boxes(
                batch_boxes, batch_probs, batch_points, img_for_box, method=self.selection_method
            )
        
        batch_boxes_dt = [b.clone().detach() for b in batch_boxes]
        # Extract faces
    ...
    def detect(self, img, landmarks=False):
        ...
        for box, point in zip(batch_boxes, batch_points):
            ...
            if len(box) == 0:
                ...
                box_order = torch.flip(torch.argsort((box[:, 2] - box[:, 0]) * (box[:, 3] - box[:, 1])), dims=(0,))
                box = box[box_order]
    ...
    def select_boxes(
        ...
        for boxes, points, probs, img in zip(all_boxes, all_points, all_probs, imgs):
            ...
            if method == 'largest':
                box_order = torch.flip(torch.argsort((boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])), dims=(0,))
            elif method == 'probability':
                box_order = torch.flip(torch.argsort(probs), dims=(0,))
            elif method == 'center_weighted_size':
                box_sizes = (boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])
                img_center = (img.width / 2, img.height/2)
                box_centers =torc.cat([((boxes[:, 0] + boxes[:, 2]) / 2).unsqueeze(-1), ((boxes[:, 1] + boxes[:, 3]) / 2).unsqueeze(-1)], dim=-1)
    ...
    def extract(self, img, batch_boxes, save_path):
        if isinstance (img, torch.Tensor) and len(img.shape)>3:
            faces = extract_face(img, batch_boxes, self.image_size, self.margin)
        else:
            batch_mode = True
            if (
                    not isinstance(img, (list, tuple)) and
